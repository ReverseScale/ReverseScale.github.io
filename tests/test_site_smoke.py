...
class SiteSmokeTest(unittest.TestCase):
    ...
    def test_pipeline_separates_execution_stages_from_entry_runtime_and_feedback(self) -> None:
        source = read("src/app-architecture.js")
        styles = read("src/app-architecture.css")

        self.assertIn("pipelineStages", source)
        self.assertIn("pipelineContexts", source)
        self.assertIn("architectureGameLevels", source)
        self.assertIn("renderPipelineStages", source)
        self.assertIn("renderPipelineEndpoint", source)
        self.assertIn("renderPipelineFeedback", source)
        ...
        self.assertIn("Pipeline 只描述可执行的交付阶段", source)
        self.assertIn("Change Set", source)
        self.assertIn("变更预检", source)
        self.assertIn("构建与验证", source)
        self.assertIn("签名与发布", source)
        self.assertIn("渐进式发布", source)
        self.assertIn("App Runtime", source)
        self.assertIn("Telemetry & Recovery", source)
        self.assertIn('renderPipelineEndpoint("source"', source)
        self.assertIn('renderPipelineEndpoint("runtime"', source)
        self.assertIn('renderPipelineFeedback()', source)
        self.assertIn('data-pipeline-context="${escapeHtml(contextId)}"', source)
        self.assertIn("pipeline-endpoint__head", source)
        self.assertIn("pipeline-node__summary", source)
        self.assertNotIn("pipeline-axis__hint", source)
        self.assertNotIn('id: "source-change"', source)
        self.assertNotIn('id: "app-runtime"', source)
        self.assertNotIn('id: "telemetry-feedback"', source)
        self.assertIn('const INITIAL_PIPELINE_STAGE_ID = "preflight"', source)
        ...
        self.assertIn("AI 生成影响证据与测试建议", source)
        self.assertIn("规则检查必须通过", source)
        self.assertIn("AI 建议不替代断言", source)
        self.assertIn("AI 只提供证据", source)
        self.assertIn("最终决定权仍由测试、预算、策略、签名与人工授权持有", source)
        self.assertEqual(source.count("inputs: ["), 4)
        self.assertEqual(source.count("actions: ["), 4)
        self.assertEqual(source.count("outputs: ["), 4)
        self.assertEqual(source.count("gates: ["), 4)
        self.assertEqual(source.count("failure:"), 4)
        self.assertEqual(source.count("signals: ["), 4)
        self.assertEqual(source.count("ai: ["), 4)
        ...
        self.assertIn(".pipeline-journey", styles)
        self.assertIn(".pipeline-endpoint", styles)
        self.assertIn(".pipeline-stage-list", styles)
        self.assertIn(".pipeline-feedback-strip", styles)
        self.assertIn(".pipeline-stage-list > li:not(:last-child)::after", styles)
        self.assertIn('.pipeline-stage-list .pipeline-node[aria-selected="true"]::before {\n  content: none;', styles)
        self.assertIn(".pipeline-detail__io", styles)
        self.assertIn(".pipeline-detail__core", styles)
        self.assertIn(".pipeline-detail__governance", styles)
        self.assertNotIn(".pipeline-flow-map", styles)
        self.assertNotIn(".pipeline-context-card", styles)
        self.assertNotIn(".pipeline-feedback-loop", styles)
        self.assertNotIn("pipelineViewport", source)
        self.assertNotIn("centerPipelineStage", source)
