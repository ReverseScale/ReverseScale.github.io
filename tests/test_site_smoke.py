...
class SiteSmokeTest(unittest.TestCase):
    ...
    def test_pipeline_defaults_to_the_first_stage_and_focuses_on_the_primary_flow(self) -> None:
        source = read("src/app-architecture.js")
        styles = read("src/app-architecture.css")

        self.assertIn("const INITIAL_PIPELINE_STAGE_ID = pipelineStages[0].id;", source)
        self.assertNotIn('const INITIAL_PIPELINE_STAGE_ID = "app-runtime";', source)
        self.assertIn('id: "source-change"', source)
        self.assertIn("centerPipelineStage(INITIAL_PIPELINE_STAGE_ID", source)
        self.assertIn("主处理链", source)
        self.assertIn("输入如何经过关键动作，转化为明确输出", source)
        self.assertIn("01 · INPUT", source)
        self.assertIn('title: "输入"', source)
        self.assertIn("02 · ACTION", source)
        self.assertIn('title: "关键动作"', source)
        self.assertIn("03 · OUTPUT", source)
        self.assertIn('title: "输出"', source)
        self.assertNotIn("pipelineControlDefinitions", source)
        self.assertNotIn("renderPipelineControl", source)
        self.assertNotIn('class="pipeline-detail__safety"', source)
        self.assertIn(".pipeline-detail__section-heading", styles)
        self.assertIn(".pipeline-detail__flow-card", styles)

    def test_pipeline_viewport_only_masks_scrollable_edges(self) -> None:
        source = read("src/app-architecture.js")
        styles = read("src/app-architecture.css")

        self.assertIn("syncPipelineViewportMask()", source)
        self.assertIn('addEventListener("scroll", this.syncPipelineViewportMask', source)
        self.assertIn('classList.toggle("is-at-start", isAtStart)', source)
        self.assertIn('classList.toggle("is-at-end", isAtEnd)', source)
        self.assertIn('dataset.scrollBoundary = "both"', source)
        self.assertIn('dataset.scrollBoundary = "start"', source)
        self.assertIn('dataset.scrollBoundary = "end"', source)
        self.assertIn('dataset.scrollBoundary = "middle"', source)
        self.assertIn(".pipeline-axis__viewport.is-at-start", styles)
        self.assertIn(".pipeline-axis__viewport.is-at-end", styles)
        self.assertIn(".pipeline-axis__viewport.is-at-start.is-at-end", styles)
        self.assertIn("-webkit-mask-image:", styles)
        self.assertIn("--pipeline-selection-inset: 32px;", styles)
        self.assertIn("padding: 22px var(--pipeline-selection-inset) 24px;", styles)
        self.assertIn("scroll-padding-inline: var(--pipeline-selection-inset);", styles)
