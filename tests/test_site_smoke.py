...
class SiteSmokeTest(unittest.TestCase):
    ...
    def test_architecture_explainer_is_written_in_simplified_chinese(self) -> None:
        ...
        self.assertIn("动态关系", source)
        self.assertNotIn("How an App learns to scale", source)
        self.assertNotIn("Everything starts close together", source)

    def test_architecture_explainer_models_five_data_driven_stages(self) -> None:
        source = read("src/app-architecture.js")

        self.assertIn('id: "monolith"', source)
        self.assertIn('id: "components"', source)
        self.assertIn('id: "modules"', source)
        self.assertIn('id: "assembly"', source)
        self.assertIn('id: "scale"', source)
        self.assertIn("architectureStages.length", source)
        self.assertIn("updateNodeSlots(stage)", source)
        self.assertIn("nodesForStage(stage)", source)
        self.assertIn("stage.links.map", source)
        self.assertIn("requestAnimationFrame", source)

    def test_components_and_modules_have_distinct_visual_boundaries(self) -> None:
        source = read("src/app-architecture.js")
        styles = read("src/app-architecture.css")

        self.assertIn("updateGroupSlots(stage)", source)
        self.assertIn("UI 组件库", source)
        self.assertIn("功能组件库", source)
        self.assertIn("通用组件 · 无业务逻辑", source)
        self.assertIn("业务模块 · 业务 UI 与业务逻辑", source)
        self.assertIn('parts: ["业务 UI", "业务逻辑", "组件依赖"]', source)
        self.assertIn("architecture-group", styles)
        self.assertIn("architecture-node__badge", styles)
        self.assertIn("architecture-node__parts", styles)
    ...
    def test_app_assembly_registers_modules_and_routes_between_them(self) -> None:
        ...
        self.assertIn("模块注册中心", source)
        self.assertIn("Router", source)
        self.assertIn("模块通信", source)
        self.assertIn("壳负责组装，Router 负责让模块相遇", source)
        self.assertIn("Router 只传路由协议，不承载业务逻辑", source)
        self.assertIn('kind: "registration"', source)
        self.assertIn('kind: "route"', source)
        self.assertIn('kind: "dependency"', source)
        self.assertIn(".architecture-flow--registration", styles)
        self.assertIn(".architecture-flow--route", styles)
        self.assertIn(".architecture-flow--dependency", styles)

    def test_scale_stage_separates_resources_config_delivery_and_telemetry(self) -> None:
        source = read("src/app-architecture.js")
        styles = read("src/app-architecture.css")

        self.assertIn("大型 App", source)
        self.assertIn("MAX_NODE_SLOTS = 7", source)
        self.assertIn("Module Manifest", source)
        self.assertIn("动态路由表", source)
        self.assertIn('parts: ["Native", "Flutter", "React Native"]', source)
        self.assertIn("动态交付 Runtime", source)
        self.assertIn("资源系统", source)
        self.assertIn("内置资源", source)
        self.assertIn("i18n", source)
        self.assertIn("动态配置 · Feature Flag · A/B Test", source)
        self.assertIn("Telemetry Runtime", source)
        self.assertIn("Log · 埋点 · 指标上报", source)
        self.assertNotIn("i18n · 动态配置 · A/B Test", source)
        self.assertIn('kind: "delivery"', source)
        self.assertIn('kind: "telemetry"', source)
        self.assertIn("architecture-capabilities", source)
        self.assertIn(".architecture-flow--delivery", styles)
        self.assertIn(".architecture-flow--telemetry", styles)
        self.assertIn(".architecture-capabilities", styles)
        self.assertIn('[data-stage-id="scale"] .architecture-node', styles)
        self.assertIn("flex-shrink: 0", styles)

    def test_horizontal_pipeline_expands_app_node_through_a_reusable_axis_contract(self) -> None:
        source = read("src/app-architecture.js")
        styles = read("src/app-architecture.css")

        self.assertIn("pipelineStages", source)
        self.assertIn("axisPanels", source)
        self.assertIn('axisId: "app-architecture"', source)
        ...
        self.assertIn("renderAxisPanel", source)
        self.assertIn("setExpandedAxis", source)
        self.assertIn("alignAxisPanels", source)
        self.assertIn("ResizeObserver", source)
        self.assertIn('data-axis-trigger="${escapeHtml(stage.axisId)}"', source)
        self.assertIn("architecture-coordinate", source)
        self.assertIn("pipeline-axis", source)
        self.assertIn("pipeline-detail", source)
        self.assertIn("architecture-axis-panel", source)
        self.assertIn("X · Delivery Pipeline", source)
        self.assertIn("Y · App Architecture", source)
        ...
        self.assertIn(".pipeline-workspace__connector", styles)
        self.assertIn(".architecture-axis-panel", styles)
        self.assertIn(".architecture-axis-stack", styles)
    ...
    def test_architecture_explainer_uses_a_bounded_heading_scale(self) -> None:
        ...
        self.assertIn("font-size: clamp(19px, 1.65vw, 24px)", styles)
    ...
    def test_architecture_explainer_uses_axis_reveal_morphing_and_flow_particles(self) -> None:
        source = read("src/app-architecture.js")
        styles = read("src/app-architecture.css")

        self.assertIn("IntersectionObserver", source)
        self.assertIn("data-pipeline-node", source)
        self.assertIn("data-axis-panel", source)
        self.assertIn("<animateMotion", source)
        self.assertIn("architecture-particle", source)
        self.assertIn("transition:", styles)
        self.assertIn("architecture-flow-pulse", styles)

    def test_architecture_explainer_has_manual_and_accessible_controls(self) -> None:
        source = read("src/app-architecture.js")
        styles = read("src/app-architecture.css")

        self.assertIn('aria-label="架构演进阶段"', source)
        self.assertIn('aria-live="polite"', source)
        self.assertIn('role="tablist"', source)
        self.assertIn('data-stage="', source)
        self.assertIn('aria-expanded="true"', source)
        self.assertIn('aria-controls="pipeline-detail axis-panel-${escapeHtml(stage.axisId)}"', source)
        self.assertIn('slot.setAttribute("aria-hidden", String(!visible))', source)
